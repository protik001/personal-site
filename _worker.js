...
async function handleMarkdownRequest(request, url, env) {
  ...
  // YAML frontmatter
  parts.push('---');
  const y = s => decodeEntities(s).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  if (title) parts.push(`title: "${y(title)}"`);
  if (description) parts.push(`description: "${y(description)}"`);
  if (canonical) parts.push(`url: "${canonical}"`);
  if (author) parts.push(`author: "${y(author)}"`);
